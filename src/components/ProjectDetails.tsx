      ...
      <div className="relative h-[40vh] md:h-[50vh] w-full overflow-hidden">
        ...
        <div className="absolute top-6 left-6 z-20">
          <Button
            variant="secondary"
            onClick={() => navigate("/")}
            className="flex items-center gap-2 bg-black/50 backdrop-blur-md text-white rounded-full hover:bg-white/20 font-medium border border-white/10"
          >
            <FaArrowLeft /> Voltar
          </Button>
        </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {project.tags.map((tag) => (
                  <span
                    key={tag}
                    className="text-sm font-mono text-white bg-white/10 px-3 py-1 rounded-full border border-white/10"
                  >
                    {tag}
                  </span>
                ))}
